...
//
//	modified by www.pengz.com 25 July 2003  - new logic for multiple languages (English, Spanish and ready for more).
...
function imgnew(ruta)
{
	imgDir.value='img/';
}

	var calendariosiguiente=
	{
			en: "Siguiente",
			es: "Siguiente",
			de: "Siguiente"
	};
	var calendarioanterior=
	{
			en: "Anterior",
			es: "Anterior",
			de: "Anterior"
	...
	var imgsrc   = new Array('pcaldrop1.gif','pcaldrop1.gif','pcalleft1.gif','pcalleft1.gif','pcalright1.gif','pcalright1.gif');
	var img      = new Array();
	var bShow    = false;
	...
	*/
	...
	if (dom) {
		...
		document.write ('<div onclick="bShow=true" id="calendar" style="z-index:+999;position:absolute;visibility:hidden;"><table width="'+((showWeekNumber==1)?250:220)+'" style="font-family:Arial;font-size:11px;border: 1px solid #A0A0A0;" bgcolor="#ffffff"><tr bgcolor="#000066"><td><table width="'+((showWeekNumber==1)?248:218)+'"><tr><td style="padding:2px;font-family:Arial;font-size:11px;"><font color="#ffffff' + '' /*C9D3E9*/ +'"><b><span id="caption"></span></b></font></td><td align="right"><a href="javascript:hideCalendar()"><img alt="'+calendariocerrar[language]+'" src="'+imgDir+'pcalclose.gif" width="15" height="13" border="0" /></a></td></tr></table></td></tr><tr><td style="padding:5px" bgcolor="#ffffff"><span id="content"></span></td></tr>');
	...
	function init() {
		if (!ns4)
		{
			...
			sHTML1 = '<span id="spanLeft" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeLeft\',\'pcalleft1.gif\');this.style.borderColor=\'#8af\';window.status=\''+scrollLeftMessage[language]+'\'" onclick="decMonth()" onmouseout="clearInterval(intervalID1);swapImage(\'changeLeft\',\'pcalleft1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartDecMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img alt="'+calendarioanterior[language]+'" id="changeLeft" src="'+imgDir+'pcalleft1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanRight" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeRight\',\'pcalright1.gif\');this.style.borderColor=\'#8af\';window.status=\''+scrollRightMessage[language]+'\'" onmouseout="clearInterval(intervalID1);swapImage(\'changeRight\',\'pcalright1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onclick="incMonth()" onmousedown="clearTimeout(timeoutID1);timeoutID1=setTimeout(\'StartIncMonth()\',500)" onmouseup="clearTimeout(timeoutID1);clearInterval(intervalID1)">&nbsp<img alt="'+calendariosiguiente[language]+'" id="changeRight" src="'+imgDir+'pcalright1.gif" width="10" height="11" border="0">&nbsp</span>&nbsp;';
			sHTML1 += '<span id="spanMonth" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeMonth\',\'pcaldrop1.gif\');this.style.borderColor=\'#8af\';window.status=\''+selectMonthMessage[language]+'\'" onmouseout="swapImage(\'changeMonth\',\'pcaldrop1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onclick="popUpMonth()"></span>&nbsp;';
			sHTML1 += '<span id="spanYear" style="border:1px solid #36f;cursor:pointer" onmouseover="swapImage(\'changeYear\',\'pcaldrop1.gif\');this.style.borderColor=\'#8af\';window.status=\''+selectYearMessage[language]+'\'" onmouseout="swapImage(\'changeYear\',\'pcaldrop1.gif\');this.style.borderColor=\'#36f\';window.status=\'\'" onclick="popUpYear()"></span>&nbsp;';
	...
	function popUpMonth() {
		if (visMonth == 1) {
			...
			offsetLeft = parseInt(crossobj.left) + 50;
			crossMonthObj.left = offsetLeft+"px";
			
			offsetTop = parseInt(crossobj.top) + 26
			crossMonthObj.top = offsetTop+"px";
	...
	function popUpYear() {
		...
		if (visYear==1) {
			...
			topOffset = parseInt(crossobj.top) + 26
			crossYearObj.top = topOffset+"px";
	...
	function constructCalendar () {
		...
		for	( datePointer=1; datePointer <= numDaysInMonth; datePointer++ ) {
			...
			if ((yearSelected < yearNow) || (monthSelected < monthNow) && (yearSelected == yearNow) || (datePointer < dateNow) && (monthSelected == monthNow) && (yearSelected == yearNow)) {
				if (enablePast == 0) {
					selDayAction = '';
					isPast = 1;
				} else {					
		...
		document.getElementById('spanMonth').innerHTML = '&nbsp;' +	monthName[language][monthSelected] + '&nbsp;<img alt="'+calendariomes[language]+'" id="changeMonth" src="'+imgDir+'pcaldrop1.gif" width="12" height="10" border="0">'
		document.getElementById('spanYear').innerHTML  = '&nbsp;' + yearSelected	+ '&nbsp;<img id="changeYear" alt="'+calendarioanio[language]+'" src="'+imgDir+'pcaldrop1.gif" width="12" height="10" border="0">';
	}

	function showCalendar(ctl, ctl2, format, lang, past, future, fx, fy) {
	...
	function showCalendarAux(dir, ctl, ctl2, format, lang, past, future, fx, fy) {
		if(dir!=imgDir) 
		{
			imgDir = dir;
			init();
		}
		showCalendar(ctl, ctl2, format, lang, past, future, fx, fy);
	}
	...
	document.onkeypress = function hidecal1 (e) {
		...
		if (KeyID == 27 || KeyID == 9) hideCalendar();
	}
